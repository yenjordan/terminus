...
class CodeExecutionService:
    ...
    async def execute_code(self, code: str, language: str, session_id: int, input_data: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        # Get session files
        # Note: We don't need to fetch files here as they're passed by the caller if needed
    ...
    async def sync_files_from_environment(self, temp_dir: str, original_files: Dict[str, str]) -> Dict[str, str]:
        """
            ...
            for root, dirs, files in os.walk(temp_dir):
                for file in files:
                    ...
                    try:
                        ...
                        # Check if file content has changed or is new
    ...
    def validate_code_security(self, code: str) -> Tuple[bool, Optional[str]]:
        ...
        for pattern in dangerous_patterns:
            if pattern in code:
                return False, f"Function call '{pattern.rstrip('(')}' is not allowed for security reasons"
    ...
    async def execute_python_code(
        ...
        # Check if code contains a class Solution and a method, but no print statement
        # This is common in coding challenges where the user expects to see output
...
"""
    ...
    async def execute_python_code_from_file(
        ...
        try:
            ...
            # Execute the code directly using subprocess
            try:
    ...
    async def _execute_python_code_docker(
        ...
        try:
            ...
            # Get memory stats (approximation)
            memory_usage_mb = 0
            try:
    ...
    async def _execute_python_code_subprocess(
        ...
        try:
            ...
            # Execute the code with resource limitations
            process = await asyncio.create_subprocess_exec(
                "python", os.path.join(temp_dir, "main.py"),
    ...
    async def _execute_python_file_subprocess(
        ...
        try:
            # Create a process to run the Python file
            ...
            try:
                ...
                try:
                    ...
                    # Process might have terminated already, which is fine
    ...
    async def execute_terminal_command(
        ...
        global DOCKER_AVAILABLE
        if DOCKER_AVAILABLE and self.docker_client:
            return await self._execute_terminal_command_docker(command, session_id, files, cwd, input_data)
        else:
            return await self._execute_terminal_command_subprocess(command, session_id, files, cwd, input_data)
    ...
    def _sanitize_terminal_command(self, command: str) -> Optional[str]:
        ...
        dangerous_patterns = [
            "rm -rf /", "rm -rf /*", ":(){ :|:& };:", "dd if=", 
            "mkfs", "fdisk", "mount", "umount", "sudo", "su",
            "passwd", "useradd", "userdel", "chmod 777",
            "wget", "curl http", "nc ", "netcat", "ssh",
            ">/dev/", "2>/dev/", "&>", "||", "&&"
        ]
        ...
        safe_commands = [
            "ls", "cat", "python", "python3", "pip", "pip3", "pip install", 
            "echo", "pwd", "head", "tail", "grep", "find", "wc", "sort", "uniq",
            "cd", "mkdir", "touch", "cp", "mv", "tree", "file", "which",
            "env", "export", "alias", "history", "clear", "help", "man",
            "nano", "vim", "emacs", "less", "more", "basename", "dirname",
            "whoami", "date", "uptime", "df", "du", "free", "ps", "top",
            "npm", "node", "git", "make", "cmake", "gcc", "g++", "javac", "java"
        ]
    ...
    def _extract_method_name(self, code: str) -> str:
        ...
        # Look for method definitions in the Solution class
        # Pattern matches: def methodName(self, ...):
        pattern = r'def\s+(\w+)\s*\(\s*self'
        matches = re.findall(pattern, code)
        
        if matches:
            # Return the first method name found
            return matches[0]
        else:
