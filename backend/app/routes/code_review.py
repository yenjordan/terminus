...
async def get_submissions(
    ...
    try:
        ...
        logger.info(f"Retrieved {len(response_submissions)} submissions for user {current_user.id} (reviewer: {is_reviewer})")
...
async def submit_review(
    ...
    try:
        ...
        logger.info(f"Review submitted by reviewer {current_user.id} for submission {submission_id}: {review_data.status}")
...
async def update_submission_status(
    ...
    try:
        ...
        logger.info(f"Submission {submission_id} status updated to {new_status} by reviewer {current_user.id}")
