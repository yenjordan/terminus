...
export const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const fileTree = React.useMemo(() => {
    ...
    // grouping files by path, keeping only the most recently updated one
    files.forEach((file) => {
  ...
  // getting all unique directories
  ...
  const handleCreateFile = () => {
    if (newFileName.trim()) {
      // determining the path based on the selected directory
      const selectedDir =
        Array.from(expandedDirectories).sort((a, b) => b.length - a.length)[0] || '/'
  ...
  // getting file icon based on file type
      ...
      {isCreatingFile && (
        <Box sx={{ p: 1, borderBottom: 1, borderColor: 'divider' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
            <Button size="small" onClick={() => setIsCreatingFile(false)} sx={{ mr: 1 }}>
              Cancel
            </Button>
            <Button
              size="small"
      ...
      <Box sx={{ flexGrow: 1, overflow: 'auto' }}>
        <List dense disablePadding>
          {directories.map((directory) => {
            ...
            const dirName =
              directory === '/' ? 'Root' : directory.split('/').filter(Boolean).pop() || ''
              ...
              <React.Fragment key={directory}>
                ...
                <Collapse in={isExpanded} timeout="auto" unmountOnExit>
                  {directoryFiles.map((file) => (
                    ...
                    >
                      <ListItemIcon sx={{ minWidth: 24 }}>{getFileIcon(file.name)}</ListItemIcon>
                      <ListItemText
                        primary={file.name}
                        primaryTypographyProps={{
                          variant: 'body2',
